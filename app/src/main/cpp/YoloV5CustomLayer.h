...
public:
    YoloV5CustomLayer(AAssetManager *mgr, const char *param, const char *bin);

    ~YoloV5CustomLayer();

    std::vector<BoxInfo> detect(JNIEnv *env, jobject image, float threshold, float nms_threshold);
    std::vector<std::string> labels{"500","100","50","10","5","1"};
private:
    ncnn::Net *Net;
    int input_size = 640;
    int num_class = 6;
    ...
    std::vector<YoloLayerData> layers{
            {"791",32,{{116,90},{156,198},{373,326}}},
            {"771",16,{{30,61},{62,45},{59,119}}},
